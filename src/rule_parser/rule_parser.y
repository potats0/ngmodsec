...
static string_list_t* append_to_string_list(string_list_t* list, char* str) {
        ...
        if (list->count >= list->capacity) {
                ...
                char** new_items =
                    g_waf_rule_malloc(new_capacity * sizeof(char*));
...
static void set_new_andbit() {
        ...
        if (current_rule_id < current_rule_mg->max_rules) {
                uint16_t current_mask =
                    current_rule_mg->rule_masks[current_rule_id]
                        .and_masks[current_sub_id];
                if (current_mask != 0) { // 如果已经有模式，生成新的 and_bit
                        uint16_t new_bit = generate_new_and_bit(current_mask);
                        if (!new_bit) {
                                fprintf(stderr,
                                        "Error: No available and_bit for rule "
                                        "%u sub_rule %u\n",
...
// 确保规则掩码数组有足够空间
static int ensure_rule_mask_capacity(sign_rule_mg_t* rule_mg,
                                     uint32_t rule_id) {
        ...
        uint32_t new_size =
            rule_id + RULESETS_GROWTH_SIZE; // 每次多分配一些空间
        rule_mask_array_t* new_masks =
            g_waf_rule_malloc(new_size * sizeof(rule_mask_array_t));
        if (!new_masks) {
        ...
        memcpy(new_masks, rule_mg->rule_masks,
        ...
        fprintf(stderr,
                "successfully reallocated rule masks array to size %d\n",
                new_size);
        return 0;
}

// 查找或创建匹配上下文
static string_match_context_t*
get_or_create_context(sign_rule_mg_t* rule_mg, http_var_type_t proto_var) {
        if (!rule_mg || !rule_mg->string_match_context_array ||
            proto_var >= HTTP_VAR_MAX || proto_var <= HTTP_VAR_UNKNOWN) {
        ...
        string_match_context_t* ctx =
            rule_mg->string_match_context_array[proto_var];
        if (ctx) {
                if (!ctx->string_patterns_list) {
                        fprintf(stderr,
                                "Context at index %d has NULL "
        ...
        ctx->string_patterns_list = g_waf_rule_malloc(
            ctx->string_patterns_capacity * sizeof(string_pattern_t));
        if (!ctx->string_patterns_list) {
...
static int ensure_patterns_capacity(string_match_context_t* ctx) {
        ...
        uint32_t new_capacity =
            ctx->string_patterns_capacity + PATTERNS_GROWTH_SIZE;
        string_pattern_t* new_list =
            g_waf_rule_malloc(new_capacity * sizeof(string_pattern_t));
        if (!new_list) {
        ...
        memcpy(new_list, ctx->string_patterns_list,
               ctx->string_patterns_num * sizeof(string_pattern_t));
...
// 查找或创建模式条目
static string_pattern_t* get_or_create_pattern(string_match_context_t* ctx,
                                               char* pattern, uint32_t flags) {
        ...
        for (uint32_t i = 0; i < ctx->string_patterns_num; i++) {
                if (ctx->string_patterns_list[i].string_pattern &&
                    strcmp(ctx->string_patterns_list[i].string_pattern,
                           pattern) == 0) {
        ...
        string_pattern_t* pattern_entry =
            &ctx->string_patterns_list[ctx->string_patterns_num];

        // 分配并复制模式字符串
        pattern_entry->string_pattern = g_waf_rule_malloc(strlen(pattern) + 1);
        if (!pattern_entry->string_pattern) {
                fprintf(stderr,
                        "Failed to allocate memory for pattern string\n");
                return NULL;
        }
...
// 添加规则关系
static int add_rule_relation(string_pattern_t* pattern_entry, uint32_t rule_id,
                             uint8_t sub_id, uint16_t and_bit) {
        rule_relation_t* new_relations = g_waf_rule_malloc(
        ...
        if (pattern_entry->relations) {
                memcpy(new_relations, pattern_entry->relations,
                       pattern_entry->relation_count * sizeof(rule_relation_t));
                g_waf_rule_free(pattern_entry->relations);
        }
        pattern_entry->relations = new_relations;

        // 添加新的relation
        pattern_entry->relations[pattern_entry->relation_count].threat_id =
            (rule_id << 8) | sub_id;
        pattern_entry->relations[pattern_entry->relation_count].pattern_id =
            pattern_entry->relation_count;
        pattern_entry->relations[pattern_entry->relation_count].and_bit =
            and_bit;
        pattern_entry->relation_count++;
...
// 主函数：添加模式到上下文
static void add_pattern_to_context(http_var_type_t proto_var, char* pattern,
                                   uint32_t flags) {
        if (!current_rule_mg) {
        ...
        printf(
            "Adding pattern: %s to %d (flags: 0x%x) for rule ID: %u "
            "(current_sub_id: %u, and_bit: 0x%x, not_mask: 0x%x)\n",
            pattern, proto_var, flags, current_rule_id, current_sub_id,
            current_and_bit, current_not_mask);
        ...
        // 获取或创建上下文
        string_match_context_t* ctx =
            get_or_create_context(current_rule_mg, proto_var);
        if (!ctx) {
                fprintf(stderr,
                        "Failed to get or create context for proto_var %d\n",
                        proto_var);
                return;
        }
        ...
        // 获取或创建模式
        string_pattern_t* pattern_entry =
            get_or_create_pattern(ctx, pattern, flags);
        if (!pattern_entry) {
        ...
        // 添加规则关系
        if (add_rule_relation(pattern_entry, current_rule_id, current_sub_id,
                              current_and_bit) != 0) {
                fprintf(stderr,
                        "Failed to add rule relation for rule %u "
                        "current_sub_id %u\n",
                        current_rule_id, current_sub_id);
                return;
        }
...
static int handle_match_expr(http_var_type_t var_type, char* pattern_str,
        ...
        char* converted_pattern __attribute__((cleanup(clenaup_ptr))) =
            convert_to_hyperscan_pattern(pattern_str, op_type, range);
...
static int handle_kvmatch_expr(hash_pattern_item_t** hash_item, char* param,
                               char* pattern_str, operator_type_t op_type,
                               uint32_t flags, substr_range_t* range) {
        ...
        if (item == NULL) {
                ...
                memset(ctx->string_patterns_list, 0,
                       ctx->string_patterns_capacity *
                           sizeof(string_pattern_t));

                // Add to hash table
                HASH_ADD_KEYPTR(hh, *hash_item, item->key, strlen(item->key),
                                item);
        }
        ...
        // 获取或创建模式
        string_pattern_t* pattern_entry =
            get_or_create_pattern(&item->context, converted_pattern, flags);
        if (!pattern_entry) {
...
static int handle_kv_string_list_expr(hash_pattern_item_t** hash_item,
                                      char* param, char** items, size_t count,
                                      uint32_t flags) {
        ...
        for (size_t i = 0; i < count; i++) {
                ...
                if (handle_kvmatch_expr(hash_item, param, item_copy, OP_EQUALS,
                                        flags, NULL) != 0) {
                        ret = -1;
                        break;
                }
...
static int handle_string_list_expr(http_var_type_t var_type, char** items,
        ...
        // 为每个字符串创建一个模式并添加到上下文
        for (size_t i = 0; i < count; i++) {
                if (handle_match_expr(var_type, items[i], OP_EQUALS, flags,
                                      range) != 0) {
                        return -1;
                }
        }
...
// 辅助函数：处理模式匹配表达式
static int handle_kv_string_list_with_context(hash_pattern_item_t** context,
                                              char* param, string_list_t* list,
                                              uint32_t flags) {
        return handle_kv_string_list_expr(context, param, list->items,
                                          list->count, flags);
}
