...
sign_rule_mg_t *dup_rule_mg(const sign_rule_mg_t *src) {
                ...
                for (uint32_t i = 0; i < src_ctx->string_patterns_num; i++) {
                    ...
                    if (src_pattern->relation_count > 0) {
                        dst_pattern->relations =
                            g_waf_rule_malloc(dst_pattern->relation_count * sizeof(rule_relation_t));
    ...
    for (int i = 0; i < HTTP_VAR_MAX; i++) {
        ...
        // 分配并复制模式列表
        dst_ctx->string_patterns_capacity = src_ctx->string_patterns_capacity;
        dst_ctx->string_patterns_list = g_waf_rule_malloc(dst_ctx->string_patterns_capacity * sizeof(string_pattern_t));
        if (!dst_ctx->string_patterns_list) {
            goto cleanup;
        }
        memset(dst_ctx->string_patterns_list, 0, dst_ctx->string_patterns_capacity * sizeof(string_pattern_t));
        ...
        if (src_ctx->string_ids) {
            ...
            memcpy(dst_ctx->string_ids, src_ctx->string_ids, src_ctx->string_patterns_num * sizeof(unsigned int));
        }
    }
