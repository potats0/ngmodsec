...
int init_rule_mg(sign_rule_mg_t *rule_mg) {
        ...
        rule_mg->rule_ids =
            g_waf_rule_malloc(rule_mg->max_rules * sizeof(uint32_t));
        if (!rule_mg->rule_ids) {
                return -1;
        }
        ...
        if (rule_mg->string_match_context_array) {
                for (int i = 0; i < HTTP_VAR_MAX; i++) {
                        ...
                        if (ctx->string_patterns_list) {
                                for (uint32_t j = 0;
                                     j < ctx->string_patterns_num; j++) {
                                        if (ctx->string_patterns_list[j]
                                                .string_pattern) {
                                                g_waf_rule_free(
...
sign_rule_mg_t *dup_rule_mg(const sign_rule_mg_t *src) {
        ...
        // 复制规则掩码数组
        if (src->rule_masks) {
                dst->rule_masks = g_waf_rule_malloc(dst->max_rules *
                                                    sizeof(rule_mask_array_t));
                if (!dst->rule_masks) {
                        fprintf(stderr, "Failed to allocate rule masks "
                                        "array\n");
                        goto error;
                }
        ...
        if (src->get_match_context) {
                ...
                HASH_ITER(hh, src->get_match_context, src_item, tmp) {
                        ...
                        if (!new_item) {
                                fprintf(stderr,
                                        "Failed to allocate hash pattern "
                                        "item\n");
                                goto error;
                        }
                        ...
                        if (src_ctx->string_patterns_list) {
                                dst_ctx->string_patterns_capacity =
                                    src_ctx->string_patterns_capacity;
                                dst_ctx->string_patterns_num =
                                    src_ctx->string_patterns_num;
                                ...
                                // 复制每个pattern
                                for (uint32_t i = 0;
                                     i < src_ctx->string_patterns_num; i++) {
                                        string_pattern_t *src_pattern =
                                            &src_ctx->string_patterns_list[i];
                                        string_pattern_t *dst_pattern =
                                            &dst_ctx->string_patterns_list[i];
                                        ...
                                        dst_pattern->hs_flags =
                                            src_pattern->hs_flags;
                                        dst_pattern->relation_count =
                                            src_pattern->relation_count;

                                        if (src_pattern->relation_count > 0) {
                                                dst_pattern->relations =
                                                    g_waf_rule_malloc(
                                                        dst_pattern
                                                            ->relation_count *
                                                        sizeof(
                        ...
                        // 添加到hash表
                        HASH_ADD_KEYPTR(hh, dst->get_match_context,
                                        new_item->key, strlen(new_item->key),
                                        new_item);
                }
        }
        ...
        // 复制每个字符串匹配上下文
        for (int i = 0; i < HTTP_VAR_MAX; i++) {
                string_match_context_t *src_ctx =
                    src->string_match_context_array[i];
                if (!src_ctx) {
                        continue;
                }
                ...
                dst_ctx->string_patterns_capacity =
                    src_ctx->string_patterns_capacity;
                dst_ctx->string_patterns_list =
                    g_waf_rule_malloc(dst_ctx->string_patterns_capacity *
                                      sizeof(string_pattern_t));
                if (!dst_ctx->string_patterns_list) {
                        goto cleanup;
                }
                ...
                for (uint32_t j = 0; j < src_ctx->string_patterns_num; j++) {
                        string_pattern_t *src_pattern =
                            &src_ctx->string_patterns_list[j];
                        string_pattern_t *dst_pattern =
                            &dst_ctx->string_patterns_list[j];

                        // 复制模式字符串
                        dst_pattern->string_pattern = g_waf_rule_malloc(
                            strlen(src_pattern->string_pattern) + 1);
                        if (!dst_pattern->string_pattern) {
                                goto cleanup;
                        }
                        strcpy(dst_pattern->string_pattern,
                               src_pattern->string_pattern);
                ...
                if (src_ctx->string_ids) {
                        dst_ctx->string_ids =
                            g_waf_rule_malloc(src_ctx->string_patterns_num *
                                              sizeof(unsigned int));
...
error:
        ...
        if (dst->string_match_context_array) {
                for (int i = 0; i < HTTP_VAR_MAX; i++) {
                        string_match_context_t *ctx =
                            dst->string_match_context_array[i];
                        if (!ctx) continue;
