...
void rule_hit_insert_value(ngx_rbtree_node_t *temp, ngx_rbtree_node_t *node,
  ...
  for (;;) {
    ...
    if (node->key != temp->key) {
      ...
      p = (n->matched_rule_id < t->matched_rule_id) ? &temp->left
                                                    : &temp->right;
    }
...
ngx_int_t insert_rule_hit_node(ngx_rbtree_t *tree, ngx_pool_t *pool,
                               ...
                               uint32_t alert_trigger_bitmask,
                               uint32_t alert_exclusion_bitmask,
                               uint32_t matched_rule_methods,
  ...
  if (existing != NULL) {
    ...
    MLOGD("finded exisesting record, rule ID: %d, Sub ID: %d, BitMask: 0x%d, "
          "not_mask: 0x%d",
          existing->matched_rule_id >> 8, existing->matched_rule_id & 0xFF,
          existing->alert_trigger_bitmask, existing->alert_exclusion_bitmask);
  ...
  MLOGD("recoed isn't exist, rule ID: %d, Sub ID: %d combined BitMask: 0x%d, "
        ...
        threat_id >> 8, threat_id & 0xFF, alert_trigger_bitmask,
        alert_exclusion_bitmask, matched_rule_methods);
...
void traverse_rule_hit_tree(ngx_rbtree_node_t *node,
  ...
  MLOGD(" tree Rule ID: %d, Sub ID: %d, BitMask: 0x%d, CombinedMask: 0x%d, "
        ...
        current->matched_rule_id >> 8, current->matched_rule_id & 0xFF,
        current->rule_hit_bitmask, current->alert_trigger_bitmask,
        current->alert_exclusion_bitmask, current->matched_rule_methods,
        current->current_request_method);
  // 需要通过xor 排除非掩码，获取真正的andbit
  if (current->rule_hit_bitmask ==
          (current->alert_trigger_bitmask ^ current->alert_exclusion_bitmask) &&
      // 非条件判断
