...
void log_rule_mg_status(sign_rule_mg_t *rule_mg) {
    ...
    MLOGN("rule_mg status: max_rules=%d, rules_count=%d", rule_mg->max_rules, rule_mg->rules_count);
    ...
    if (rule_mg->rules_count > 0) {
        ...
        for (uint32_t i = 0; i < rule_mg->rules_count; i++) {
            ...
            if (rule_mg->string_match_context_array) {
                MLOGN("    Rule AndBits:");
                for (int ctx_idx = HTTP_VAR_UNKNOWN + 1; ctx_idx < HTTP_VAR_MAX; ctx_idx++) {
                    ...
                    for (uint32_t pat_idx = 0; pat_idx < ctx->string_patterns_num; pat_idx++) {
                        ...
                        for (uint32_t rel_idx = 0; rel_idx < pattern->relation_count; rel_idx++) {
                            ...
                            if ((rel->threat_id >> 8) == rule_id) {
                                ...
                                snprintf(threat_id_str, sizeof(threat_id_str), "%u", rel->threat_id);
                                snprintf(pattern_id_str, sizeof(pattern_id_str), "%u", rel->pattern_id);
            ...
            for (uint8_t sub_id = 0; sub_id < masks->sub_rules_count; sub_id++) {
                ...
                snprintf(and_mask_str, sizeof(and_mask_str), "0x%04x", masks->and_masks[sub_id]);
                snprintf(not_mask_str, sizeof(not_mask_str), "0x%04x", masks->not_masks[sub_id]);
    ...
    if (rule_mg->string_match_context_array) {
        ...
        for (int i = HTTP_VAR_UNKNOWN + 1; i < HTTP_VAR_MAX; i++) {
            ...
            for (uint32_t j = 0; j < ctx->string_patterns_num; j++) {
                ...
                snprintf(hs_flags_str, sizeof(hs_flags_str), "0x%04x", pattern->hs_flags);
