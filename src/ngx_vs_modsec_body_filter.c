...
static ngx_int_t ngx_http_modsecurity_body_filter(ngx_http_request_t *r, ngx_chain_t *in) {
    ...
    set = (waf_sign_template_t *)wafconf_general_conf_get_by_server_id(ngx_server_id, CONF2_VIRTUAL_SITE_SUB_EVENT);

    /* 云端只引用自定义模板情况 */
    if (NULL == set) {
        set = (waf_sign_template_t *)wafconf_general_conf_get_by_server_id(ngx_server_id,
                                                                           CONF2_VIRTUAL_SITE_SUB_CUSTOM_EVENT);
    }
    if (NULL == set) {
        return ngx_http_next_body_filter(r, in);
    }
    ...
    for (cl = in; cl; cl = cl->next) {
        ...
        if (r->upstream && r->upstream->conf && usrdata->rsp_detect_len > r->upstream->conf->proxy_detect_body_size) {
            return ngx_http_next_body_filter(r, in);
        }
