...
int compile_hyperscan_database(string_match_context_t *ctx) {
    ...
    unsigned int *ids = g_waf_rule_malloc(ctx->string_patterns_num * sizeof(unsigned int));
    ...
    for (uint32_t i = 0; i < ctx->string_patterns_num; i++) {
        ...
        flags[i] = get_hyperscan_flags(ctx->string_patterns_list[i].hs_flags) | HS_FLAG_SOM_LEFTMOST;
        ids[i] = i; // 使用索引作为ID
    }

    // 编译数据库
    hs_compile_error_t *compile_err = NULL;
    if (hs_compile_multi(patterns, flags, ids, ctx->string_patterns_num, HS_MODE_BLOCK, NULL, &ctx->db, &compile_err) !=
        HS_SUCCESS) {
        if (compile_err) {
...
int compile_all_hyperscan_databases(sign_rule_mg_t *rule_mg) {
    ...
    if (rule_mg->headers_match_context) {
        ...
        HASH_ITER(hh, rule_mg->headers_match_context, current, tmp) {
            string_match_context_t *ctx = &current->context;
            if (compile_hyperscan_database(ctx) != 0) {
                fprintf(stderr, "Failed to compile Hyperscan database for HEADER arg %s\n", current->key);
                return -1;
            }
        }
    }
