...
ngx_vs_modsec_ctx_t *ngx_http_modsecurity_get_ctx(ngx_http_request_t *r) {
    ...
    ngx_vs_modsec_ctx_t *ctx = ngx_http_get_module_ctx(r, ngx_http_modsecurity_module);
    if (ctx == NULL) {
...
// rule指令处理函数
static char *ngx_http_modsecurity_rule(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
    ...
    if (cf->args->nelts != 2) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "invalid number of arguments in rule directive: %d", cf->args->nelts);
...
// 模块指令定义
static ngx_command_t ngx_http_modsecurity_commands[] = {{ngx_string("rule"), NGX_HTTP_LOC_CONF | NGX_CONF_ANY,
...
ngx_module_t ngx_http_modsecurity_module = {NGX_MODULE_V1,
