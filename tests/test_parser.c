...
static int on_match(unsigned int id, unsigned long long from, unsigned long long to, unsigned int flags,
                    void *context) {
...
TEST_CASE(starts_with_basic) {
    ...
    ASSERT(ctx->string_patterns_list[0].string_pattern != NULL, "Pattern string is NULL");
...
TEST_CASE(multi_pattern_hyperscan) {
    ...
    const char *rule_str = "rule 4001 http.uri contains \"admin\" and http.uri contains \"php\";";
    sign_rule_mg_t *rule_mg = calloc(1, sizeof(sign_rule_mg_t));
    ASSERT_NOT_NULL(rule_mg, "Failed to allocate rule_mg");
    ASSERT_EQ(0, init_rule_mg(rule_mg), "Failed to initialize rule_mg");
    ...
    for (int i = 0; i < HTTP_VAR_MAX; i++) {
        string_match_context_t *ctx = rule_mg->string_match_context_array[i];
        if (ctx && ctx->db) {
            ret = hs_scan(ctx->db, test_url, strlen(test_url), 0, scratch, on_match, NULL);
...
TEST_CASE(http_method) {
    ...
    const char *rule_str = "rule 8964 http.uri contains \"a\" and http.method = GET|POST;";
    sign_rule_mg_t *rule_mg = calloc(1, sizeof(sign_rule_mg_t));
    ASSERT_NOT_NULL(rule_mg, "Failed to allocate rule_mg");
    ASSERT_EQ(0, init_rule_mg(rule_mg), "Failed to initialize rule_mg");
    ...
    ASSERT(0xFFFFFFFF == rule_mg->rule_masks[10002].method[0], "wrong method id 0xFFFFFFFF");

    // 确保边界值的method也被初始化为0xFFFFFFFF
    ASSERT(0xFFFFFFFF == rule_mg->rule_masks[rule_mg->max_rules - 1].method[0], "wrong method id 0xFFFFFFFF");

    ASSERT(0xFFFFFFFF == rule_mg->rule_masks[10120].method[0], "wrong method id 0xFFFFFFFF");

    // 确保扩容前的已设置的method正常
    ASSERT(10 == rule_mg->rule_masks[8964].method[0], "wrong method id 8964");
    // 确保扩容前未设置的method正常
    ASSERT(0xFFFFFFFF == rule_mg->rule_masks[1984].method[0], "wrong method id 8964");
    destroy_rule_mg(rule_mg);
    passed_tests++;
}
...
TEST_CASE(quote_in_rule) {
    ...
    // 检查模式是否正确（\" a 应该被解析为 " a）
    ASSERT_STR_EQ("^\" a$", ctx->string_patterns_list[0].string_pattern, "Pattern mismatch");
